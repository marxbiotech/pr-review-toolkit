...
**Canonical taxonomy invariant.** All actionable Codex findings — those that
require action before merge — MUST be inserted into the existing canonical
sections (`### 🔴 Critical Issues`, `### 🟡 Important Issues`, `### 💡 Suggestions`)
with sequential renumbering, and the corresponding `issues.{cat}.total` counter
in metadata MUST be incremented. Parallel sections (e.g. `### 🟠 Codex Follow-up
Notes`) are reserved for non-canonical content: validation confirmations,
environmental observations, and audit trails that do not change merge status.

Rationale: the Summary table and `pr-review-resolver`'s default discovery flow
both rely on canonical sections as the single source of truth for unresolved
work. Parallel actionable blocks cause counter drift and resolver blind spots.
